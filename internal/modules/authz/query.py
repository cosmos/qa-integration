...
def query_authz_grantee_grants(grantee):
    command = f"{DAEMON} q authz grantee-grants {grantee} --node {RPC} --chain-id {CHAINID} --output json --count-total"
    return exec_command(command)
...
def query_authz_granter_grants(granter):
    command = f"{DAEMON} q authz granter-grants {granter} --node {RPC} --chain-id {CHAINID} --output json --count-total"
    return exec_command(command)
