import os
from utils import exec_and_process_output
...
def query_deposits(proposal_id, extra_args=""):
    command = f"{DAEMON} q gov deposits {proposal_id} --node {RPC} --chain-id {CHAINID} --output json"
    return exec_and_process_output(command, extra_args)


# query_deposit queries details of a deposit with specific deposit address and proposal id
def query_deposit(proposal_id, deposit_addr, extra_args=""):
    command = f"{DAEMON} q gov deposit {proposal_id} {deposit_addr} --node {RPC} --chain-id {CHAINID} --output json"
    return exec_and_process_output(command, extra_args)


# query_params queries the parameters of the governance process
def query_params(extra_args=""):
    command = f"{DAEMON} q gov params --node {RPC} --chain-id {CHAINID} --output json"
    return exec_and_process_output(command, extra_args)


# query_param queries the specific parameter (voting|tallying|deposit of the governance process
def query_param(param_type, extra_args=""):
    command = f"{DAEMON} q gov param {param_type} --node {RPC} --chain-id {CHAINID} --output json"
    return exec_and_process_output(command, extra_args)
...
def query_proposal(proposal_id, extra_args=""):
    command = f"{DAEMON} q gov proposal {proposal_id} --node {RPC} --chain-id {CHAINID} --output json"
    return exec_and_process_output(command, extra_args)


# query_proposer queries address who proposed proposal
def query_proposer(proposal_id, extra_args=""):
    command = f"{DAEMON} q gov proposer {proposal_id} --node {RPC} --chain-id {CHAINID} --output json"
    return exec_and_process_output(command, extra_args)
